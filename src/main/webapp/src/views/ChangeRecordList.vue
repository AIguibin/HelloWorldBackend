<template>
  <div style="padding:16px;">
    ...
    <el-table :data="list" border stripe :header-cell-style="headerCellStyle">
      ...
      <!-- 问题与方案 -->
      <el-table-column prop="problemDescription" label="问题描述" show-overflow-tooltip="true"/>
      <el-table-column prop="impactAnalysis" label="影响分析" show-overflow-tooltip="true"/>
      ...
      <el-table-column prop="codeList" label="代码清单" show-overflow-tooltip="true"/>
...
export default {
  ...
  data() {
    return {
      ...
      search: { groupName: '', developer: '', serviceName: '', defectNumber: '' },
      searchRange: [],
  ...
  methods: {
    ...
    async onDownload() {
      try {
        const params = { currentStatus: this.downloadStatus, limit: 1000 };
        if (this.searchRange && this.searchRange.length === 2) {
