...
- **环境感知**: 处理请求前，先通过 `pom.xml`、`src/main/webapp/package.json` 识别技术版本；自动区分后端（`src/main/java`）、前端（`src/main/webapp`）、脚本（`scripts/`）、迁移脚本（`src/main/resources/db/migration`）、文档（`docs/`）目录。
...
- **构建工具固定**:
  - 后端一律使用项目根目录的 `./mvnw`（Maven 官方标准位置）。**禁止使用本机 mvn**——本机 3.6.0 无法解析 Boot 4 的 pom（model 4.1.0）。
...
- **迁移脚本**: 表结构/初始数据/增量变更一律放 `src/main/resources/db/migration/V<N>__描述.sql`（Flyway），禁止修改已合入的历史脚本。
...
- **目录职责**: 根目录 `mvnw`/`mvnw.cmd`/`.mvn/wrapper/` 按 Maven 官方标准；根目录 `Jenkinsfile` 按 Jenkins 标准；`scripts/` 放运维/工具脚本（restart、start、端口检查等）；`docs/` 放设计文档。
...
2. Entity + RO（入参）+ VO（出参），位于 `module/<模块名>/` 各子包
